...
ApplicationWindow {
    ...
    ColumnLayout {
		...
		// ===== BEGIN: Drone Flight Log glue =====

// Model for the log
ListModel { id: droneFlightModel }

// Bind backend signal -> model
Connections {
    target: backend
    function onFlightLogUpdated(entries) {
        droneFlightModel.clear();
        for (var i = 0; i < entries.length; ++i) {
            droneFlightModel.append({ text: entries[i] });
        }
    }
}
...
GroupBox {
    title: "Flight Log"
    // If parent uses Layouts, these apply; safe to keep either way
    Layout.fillWidth: true
    Layout.fillHeight: true

    // Children use a Layout (no anchors inside Row/Column parents)
    ColumnLayout {
        anchors.fill: parent    // parent is GroupBox, so this is fine
        spacing: 8

        ListView {
            id: droneFlightLogView
            Layout.fillWidth: true
            Layout.fillHeight: true
            clip: true
            model: droneFlightModel
            delegate: Text {
                text: model.text
                wrapMode: Text.Wrap
                font.pixelSize: 14
        ...
        RowLayout {
            ...
            Button {
                text: "Clear Log"
                onClicked: droneFlightModel.clear()
            }
        }
    }
}
// ===== END: Drone Flight Log glue =====
